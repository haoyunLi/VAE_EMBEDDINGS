...
def remove_gene_version(gene_id: str) -> str:
    """
    Remove version number from gene ID 
...
def load_common_genes(common_genes_path: str) -> List[str]:
    """
    Load common gene IDs from a text file and remove version numbers.
    
    @param common_genes_path: Path to the common gene IDs file
    @return: List of common gene IDs without version numbers
    """
    with open(common_genes_path, "r") as f:
        return [remove_gene_version(line.strip()) for line in f.readlines()]
...
if __name__ == "__main__":
    ...
    common_genes_path = 'data/ensg_coding_genes.txt'
